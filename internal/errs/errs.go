// Package errs 定义跨层的稳定错误码与操作上下文错误。本包为叶子包，
// 只被向上消费（门面 / 域 / 工具），不反向依赖域实现。
package errs
...
// Unwrap，因此 errors.Is(err, ErrNotFound) 等判断可用；定位信息（Part /
// NodePath / SlideID / ShapeID / Message）通过 errors.As 读取。
