# go-pptx 组件完整设计方案 V2.6

> 文档类型：开发实施基线｜日期：2026-09-08｜版本：V2.6
> V2.6 = V2.5 开发实施基线 + 评审报告（`go-pptx-设计方案-V2.5-评审报告.md`）P0/P1/P2 问题整改（变更明细见附录 A）
...
**阅读导航：** 1–5 章定义范围、架构与核心接口（2.3 为 OOXML 特性能力矩阵及其工作包映射，2.4 为差异化能力路线）；6–14 章定义文档能力；15–17 章定义验收与阶段；18–23 章提供实现细节和扩展接口；24–28 章可直接用于拆分任务、编写测试与集成开发；29 章列参考依据。
...
**M6 特性到工作包的映射（V2.6 新增，评审 1.2，P1）**：此前版本的矩阵精确声明了约 30 项特性的目标档位与阶段，但 24 章工作包 backlog 未逐项承接，存在"矩阵存在承诺、无人认领执行"的风险。V2.6 按特性类别归并为若干 M6 工作包，做到矩阵每一行都能追溯到具体任务；工作包定义见 24 章，此处只做映射声明：

| 矩阵特性类别 | 归并工作包 |
|---|---|
| 预设几何全集、自定义几何、效果模型、渐变/图案填充、线条系统 | GEOM-02 |
| 样式矩阵、颜色变换全集 | STYLE-02 |
| 章节/自定义放映、嵌入字体识别、讲义母版/视图属性、避头尾规则 | LAYOUT-01 |
| 过渡动画（R 及基础 E） | ANIM-02 |
| 视频媒体形状 | VIDEO-01 |
| 图表标签/误差线/趋势线/轴扩展 | CHART-02 |
| 文本框高级项、字段全集逐类型 | TEXT-03 |

未归并到具体工作包之前，M6 阶段的相关特性视为"目标已声明、排期未分解"状态，不计入任何已完成或已排期的宣称。

### 2.4 差异化能力路线

2.3 表达"相对完整 OOXML 的覆盖目标"，本节声明相对同类组件（如 python-pptx）的**差异化目标**。原则：不做同质化功能堆叠，投入集中在三类能力——竞争者架构性做不到的、能开辟新场景的、以及明确不做的。本节是后续新增能力的立项过滤器：任何新能力必须强化下述叙事主线之一，否则不立项。

**（1）差异化护城河**——在 M0–M5 期间随底座自然形成，不设独立里程碑：

| 能力 | 内容 | 依托章节 |
|---|---|---|
| 词法保真补丁引擎 | 公开承诺"修改处之外字节不变"的可审计 API（SaveReport + 补丁保证声明）；对企业存量模板中的未知扩展、宏残留、WPS 私有节点实现结构性保留。python-pptx 的 lxml 全量解析路线在架构上无法追随 | 3、4、12、19 |
| 能力协商协议 | 六维能力报告额外输出为带 schemaVersion 的机器可读 capability manifest，调用方实现能力感知降级与编排集成；配套稳定错误码。全生态无同类实现 | 12.2、20.4、23.2 |
| 服务端原生基因 | ReaderAt 流式加载与大媒体不落盘、资源预算 + fuzz 加固（可安全处理不可信 PPTX）、单实例串行 + revision 事务模型；只读并发快照能力作为候选补充项，见 ADR 015（待评估，不改变本版本单实例串行的既定结论） | 5、13、19 |
| 浏览器原生检查工具 *(V2.6 新增，评审 3.1)* | "纯 Go、CGO_ENABLED=0"约束天然支持 `GOOS=js GOARCH=wasm` 编译，据此把只读 CLI 能力（inspect/capability/validate）交付为文件不出本地设备的浏览器端检查工具；office_oxide 的 WASM 包装 Rust 产物、python-pptx 完全无法运行于浏览器，均无法复用此路径 | 23.2、26、TOOL-02 |

**（2）创新方向**——M7/M8 创新阶段逐项立项，不改变 M0–M2 的 MVP 优先级：

| 创新项 | 内容 | 依赖 | 阶段 |
|---|---|---|---|
| 动画时序只读 IR | 把 `p:timing` 投影为只读时序 IR（每形状入场/强调/退出时间轴、触发条件、与配音的交互点），回答"这页 PPT 的动画时间轴是什么"；不做动画编辑。规格见 21.5 | AUDIO-02 | M7 |
| 模板数据绑定引擎 | 声明式模板层：占位符 + 数据源 → 整套文档，含表格行循环、条件段落、图表数据绑定；取代调用方自行拼装 Jinja/python-pptx 类方案。必须构建在保真补丁与跨 Run 替换之上（ADR 013） | TEXT-02、CHART-01 | M7 |
| 能力 manifest 工具链 | `pptx capability` 命令与 JSON schema 发布，护城河第 2 项的工具化交付 | TOOL-01 | M7 |
| WASM 浏览器检查工具 *(V2.6 新增，评审 3.1)* | 把 TOOL-01 的只读命令编译为 WASM，交付零上传的静态网页检查工具，服务企业客户上传前自查敏感/未知内容的隐私诉求 | TOOL-01、26 章 WASM 编译验证 | M7 |
| 语义 Diff 与审计 | 两份 PPTX 的结构化差异报告（文字/顺序/样式变更 + opaque 区域标记），服务审批流、版本管理与合规审计。规格随 18.3 | TIMIR-01 | M8 |
| 有限原生预览 | 标注覆盖度的"高质量缩略图"渲染（常用 preset 几何子集 + 基础文本布局 + 纯色/图片填充），能力清单随版本公开；不承诺 V1。定位见 23.1 | GEOM-01、STYLE-01 | M8 |
| 讲稿标记语法子包 *(V2.6 新增，评审 3.2，候选，非既定决策)* | 可选 `narration` 子包：文档化的停顿/强调标记约定 + 备注分段解析 + SSML 片段生成，服务第 22 章已声明的自动讲解场景；与 ADR 009"渲染/TTS/AI 适配独立"存在边界张力，是否采纳需团队专项评审，本版本不默认立项 | 待评审 | 待评审 |

**（3）明确不做**（与 2.3 矩阵档位一致）：完整动画编辑、像素级渲染复刻、chartEx 现代图表、SmartArt 布局、Strict 格式语义写入、宏写入。

**叙事主线**：保真（"改过的东西之外一个字节不动"）、可靠（恶意文件炸不掉的并发服务端组件）、闭环（从模板到配音成片的一条 Go 原生流水线）。
...
> **V2.6 新增（评审 4.1，P0）**：2.1 节把"根据模板批量生成报告、方案和培训材料"列为第一核心场景，批量生成场景下逐份设置文档标题、作者、公司等属性是必然需求，此前版本遗漏此接口，V2.6 补齐。

```go
type CoreProperties struct {
    Title, Subject, Author, Company, Keywords, Category, Comments Optional[string]
    Created, Modified Optional[time.Time]
}
type CorePropertiesPatch = CoreProperties // 与 FontStyle 一致的 Optional patch 语义
...
- 分别对应 `docProps/core.xml`（标题/作者/主题等标准属性）与 `docProps/custom.xml`（自定义键值属性）；`docProps/app.xml`（应用统计信息，如页数、字数）首版只读识别，不提供写入 API，因其数值通常由生成客户端在编辑时自动维护，程序写入容易与实际内容不一致。
- `SetCoreProperties` 采用与 `FontStyle` 一致的 `Optional[T]` patch 语义：仅更新 `Set=true` 的字段，不清空未提及字段；`Modified` 时间戳默认由库在保存时自动更新为当前时间，除非调用方显式传入。
- 缺少 `docProps/core.xml`（少数非标准生成器产物）时，读取返回零值 `CoreProperties` 和 nil error；写入时按需创建该 Part 及其 Content Type 声明与关系条目。
- `CustomPropertyValue` 支持字符串、数值、布尔、日期四种基础类型，对应 OOXML custom properties 的 `lpwstr`/`i4`/`bool`/`filetime` 变体；不支持的复杂类型返回 `ErrUnsupportedFormat`。
...
> **V2.6 新增（评审 4.2，P1）**：替代文本是企业客户合规审查中常见的检查项，实现成本低，此前版本遗漏。

`PictureSpec`、`AutoShapeSpec` 均包含 `AltText string` 与 `IsDecorative bool` 字段，对应 `a:cNvPr` 的 `descr` 属性与装饰性图形标记；`BaseShape` 提供对应只读访问：

```go
func (s BaseShape) AltText() string
func (s BaseShape) IsDecorative() bool
func (s *PictureShape) SetAltText(text string) error
func (s *PictureShape) SetDecorative(decorative bool) error
```

`IsDecorative=true` 时写入规范定义的装饰性标记而非空替代文本，二者在辅助技术读取语义上不同，不能用空字符串代替显式装饰性声明。读取时若原文件两者均未声明，返回空值和 `Set=false`，不臆测。
...
用 `Inspect / Create / Edit / Preserve / Render / Play` 六维状态替代单个 FullySupported 标签。状态为 Supported、Partial、Unsupported、Untested，并附限制和适用客户端。能力报告同时提供机器可读的 **capability manifest**（带 schemaVersion 的 JSON，见 2.4 护城河第 2 项），供调用方实现能力感知的降级与编排逻辑；manifest 的 schemaVersion 与 SDK 版本独立管理。
...
> **V2.6 新增说明（评审 1.3，P2）**：本章及第 25 章给出的用例、阈值、数值（如 AT-06 的 10s/8s 时长组合）是**测试设计规格**，用于指导测试用例编写与自动化实现，不代表已被执行或已通过；具体数值、样本状态是否已验证，以 15.3 节"通过数/样本数"披露和实际 CI 报告为准。文档全篇的精确度是为了让规格可直接转化为测试代码，不应被解读为功能已实现的证明。
...
| M6 扩展阶段 | 跨文档合并、Strict、更多客户端、渲染适配，及 GEOM-02/STYLE-02/LAYOUT-01/ANIM-02/VIDEO-01/CHART-02/TEXT-03（2.3 矩阵 M6 目标特性的工作包分解，见 24 章） | 各功能独立验收发布 | 单独估算 |
| M7 创新扩展 I | 动画时序只读 IR（21.5）、模板数据绑定、能力 manifest、WASM 浏览器检查工具（TOOL-02） | 依赖 M2、AUDIO-02、TOOL-01；逐项独立验收（见 2.4） | 单独估算 |
| M8 创新扩展 II | 语义 Diff 与审计、有限原生预览（高质量缩略图） | 依赖 TIMIR-01、GEOM-01、STYLE-01；逐项独立验收（见 2.4） | 单独估算 |

**M0–M2 约 10–16 人周，形成首个可用 MVP；M0–M5 合计约 22–38 人周，未含 M6。** M7/M8 为创新扩展阶段（见 2.4），单独估算与排期，不改变 MVP 优先级；创新项必须在对应底座工作包验收通过后立项。关键技术验证失败时先调整保存方案，避免上层功能建立在不可保真的底座上。M4 可在 M2 完成后按业务优先级前移，但不能绕过计时保留验证。
...
| 011 | 能力报告同时提供机器可读 capability manifest | 支持调用方能力感知、降级与编排集成 |
| 012 | 动画只做时序只读 IR，不做编辑 | 编辑需时间求值器与行为库全表，投入产出比不足（2.4） |
| 013 | 模板数据绑定构建在保真补丁与跨 Run 替换之上 | 不引入第二套编辑路径，避免双写分歧（与 ADR 002 一致） |
| 014 | 原生渲染定位为标注覆盖度的高质量缩略图 | 不承诺 V1，避免与像素级复刻混同（23.1） |
| 015（候选，未采纳） | 是否新增 `Presentation.Snapshot()` 只读并发视图 | 单实例串行（ADR 未单列）在多用户并发只读预览场景下需业务层自行加锁或重复 Open；快照按 revision 冻结一份只读状态，不改变现有写路径的串行约束。评审 2.3 提出，需团队评估工程量与实际并发读取场景的优先级后再决定是否转为正式 ADR |
...
`ir` 是可选只读包，依赖公共 `pptx`，核心不得导入 `ir`。IR 包含 schemaVersion、documentID、页面顺序、对象 ID、类型、文字、备注、变换、已解析格式、媒体引用和能力诊断。IR 可选包含动画时序投影（见 21.5），并作为两份文档语义 diff 的输入视图；diff 输出将未识别区域标记为 opaque diff，不猜测其内部变化。
...
### 21.5 动画时序只读 IR（M7 创新扩展）

本节实现 2.4 的"动画时序只读 IR"：目标档位为 2.3 矩阵中动画对象模型 R 档的前置交付。**不提供任何动画编辑 API**，只把 `p:timing` 投影为只读时序 IR，供调用方（尤其是自动讲解视频、AI 内容理解场景）回答：

- 每个形状的入场/强调/退出动画类型与触发条件（onClick/onBegin/延迟）；
- 页面级动画时间轴（输出标记为 Estimated 的估计值，不承诺与客户端播放帧一致）；
- 与配音音轨的时序交互点（音频 begin 条件落在哪个动画序列之后）。

实现约束：复用 AUDIO-02 的受限解析与时间节点 ID 识别；未识别的动画节点输出为 opaque node 并计入诊断，不猜测、不省略，如实报告 Partial/Untested 状态。本项不改变 21.3 的结论——在时间求值器立项前，PlanTimingSync 仍按受支持配置检测并限制；时序 IR 的估计值不得作为 AdvanceAfter 的计算输入。
...
纯 Go 原生预览若启动，单独立项并定义文本 shaping、CJK 换行、字体缓存、嵌入字体处理及支持覆盖表。首期定位为**标注覆盖度的"高质量缩略图"**（常用 preset 几何子集、基础文本布局、纯色/图片填充），能力清单随版本公开（ADR 014）。该项目不阻断核心发布，不承诺 V1，也不以完整 Office 视觉兼容作为短期承诺。
...
| `pptx capability input.pptx --json` | 输出 capability manifest（12.2） | 只读，不修改文件 |
| `pptx diff input-a.pptx input-b.pptx --output report.json` | 输出两份文档语义差异报告 | 未识别区域标记为 opaque diff；不修改输入（M8） |
...
| CAP-01 | capability manifest 导出与 JSON schema | TOOL-01 | manifest schema 稳定，六维状态与 SaveReport/能力报告一致 |
| TIMIR-01 | 动画时序只读 IR（21.5） | AUDIO-02 | 动画金样时间轴输出正确，未识别节点 opaque 标记与诊断齐全 |
| TPL-01 | 模板数据绑定引擎 | TEXT-02、CHART-01 | 行循环/条件段落/图表绑定金样通过，绑定失败显式报错且不产生部分写入 |
| DIFF-01 | 语义 diff 与审计报告 | TIMIR-01 | diff 报告含 opaque 区域且定位可回溯到 Part/NodePath |
| GEOM-02 *(V2.6 新增)* | 预设几何全集/自定义几何/效果模型/渐变图案填充/线条系统（2.3 矩阵映射） | GEOM-01 | 各子特性达到矩阵声明档位，R 档以诊断输出验证、E 档以金样验证 |
| STYLE-02 *(V2.6 新增)* | 主题样式矩阵、颜色变换全集（2.3 矩阵映射） | STYLE-01 | `styleMatrixReference` 引用链与约 20 种颜色变换的解析结果通过语料回归 |
| LAYOUT-01 *(V2.6 新增)* | 章节/自定义放映、嵌入字体识别、讲义母版/视图属性、避头尾规则（2.3 矩阵映射） | MODEL-01 | 四项特性均达到 R 档：可解析并输出诊断，不提供写入 API |
| ANIM-02 *(V2.6 新增)* | 过渡动画 `p:transition`（含 `p14:morph`）R 档及基础 E 档（2.3 矩阵映射） | AUDIO-02 | 常见过渡类型可读取，基础过渡类型的受限写入不破坏原 timing 树 |
| VIDEO-01 *(V2.6 新增)* | 视频媒体形状，复用 MEDIA/AUDIO 机制（2.3 矩阵映射） | MEDIA-01、AUDIO-01 | 视频嵌入与 poster frame 处理达到 E 档，客户端可播放 |
| CHART-02 *(V2.6 新增)* | 图表标签、误差线、趋势线、日期轴/对数轴扩展（2.3 矩阵映射） | CHART-01 | 扩展信息达到 R 档，读取结果与客户端显示一致 |
| TEXT-03 *(V2.6 新增)* | 文本框高级项（`numCol`/`vert`/`anchorCtr`）、字段全集逐类型（2.3 矩阵映射） | TEXT-02 | R 档覆盖全部字段类型，E 档子集通过金样 |
| TOOL-02 *(V2.6 新增，评审 3.1)* | WASM 编译目标与浏览器端只读检查工具 | TOOL-01、26 章 WASM CI 验证 | 核心包 WASM 编译通过 CI；静态网页可离线加载并输出与 CLI 一致的 inspect/capability 结果，文件不出本地设备 |

每个工作包提交：实现代码、公开接口 GoDoc、最小使用示例、正反向测试、相关语料许可记录、能力矩阵更新。不能以“接口已定义”或“XML 已生成”作为功能完成标准。GEOM-02/STYLE-02/LAYOUT-01/ANIM-02/VIDEO-01/CHART-02/TEXT-03 七项均属 M6 阶段，具体排期顺序按语料出现频率驱动，不要求按本表顺序执行。
...
- **V2.6 新增（评审 2.2，P1）**：每次发布 CI 额外验证 `GOOS=js GOARCH=wasm` 能否成功编译核心包（`pptx` 根包及只读子集）。该验证不引入额外开发工作量，只用于兑现"纯 Go、零 cgo"约束已隐含的浏览器端可运行能力，编译失败视为对既有架构约束的回归，而非可选项。此验证只覆盖可编译性，不代表已交付面向用户的浏览器端功能（对应能力见 2.4 节创新方向表 WASM 检查工具项与 TOOL-02 工作包）。
...
| 版本 | 日期 | 变更 |
|---|---|---|
| V2.3 | 2026-09-07 | 开发实施基线 |
| V2.5 | 2026-09-07 | 新增 2.3 OOXML 特性能力矩阵（P/R/E/F 四档 × 30 项特性）；新增 2.4 差异化能力路线（护城河/创新方向/不做清单/立项过滤器）；12.2 增加 capability manifest 输出；18.3 IR 扩展动画时序投影与语义 diff 语义；新增 21.5 动画时序只读 IR 规格；23.1 明确有限原生预览（高质量缩略图）定位；23.2 新增 `capability`/`diff` CLI 命令；16 章新增 M7/M8 创新扩展阶段；24 章新增 CAP-01/TIMIR-01/TPL-01/DIFF-01 工作包；17 章新增 ADR 011–014 |
| V2.6 | 2026-09-08 | 整改评审报告（`go-pptx-设计方案-V2.5-评审报告.md`）全部 P0/P1 问题及部分 P2 候选项：【功能完整性 P0】新增 5.1 文档级元数据（docProps）读写 API，对应核心场景#1；【功能完整性 P1】新增 8.1 无障碍替代文本（AltText/IsDecorative）；【逻辑合理性 P1】2.3 矩阵新增 M6 特性到工作包的映射表，24 章新增 GEOM-02/STYLE-02/LAYOUT-01/ANIM-02/VIDEO-01/CHART-02/TEXT-03 七个工作包，闭合矩阵与 backlog 的可追溯性缺口；【架构先进性 P1】26 章新增 WASM 可编译性 CI 验证；【产品创新性 P1】2.4 护城河新增"浏览器原生检查工具"、创新方向新增对应 M7 交付项，24 章新增 TOOL-02 工作包；【逻辑合理性 P2】15 章开头新增设计态/验证态区分说明；【架构先进性 P2，候选未采纳】17 章新增 ADR 015（只读并发快照，标注待评估）；【产品创新性 P2，候选未采纳】2.4 创新方向新增"讲稿标记语法子包"候选项，标注与 ADR 009 存在边界张力、需团队专项评审 |
