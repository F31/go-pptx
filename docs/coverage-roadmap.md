...
- **Evening refresh (ADR-017 第三批 chart 全实现搬迁)**: chart 的 parse / build / canonical / validate / fragment / workbook 实现搬到 `internal/chart`。**归属陷阱**：实现搬走后若白盒测试仍留在根包，per-package 覆盖率不再归属被执行语句——`internal/chart` 记 5.5%、全仓 total 一度掉到 **79.4%**（跌破 COV-01 的 80% 里程碑）。处置：① 把 5 个直接断言本包语义的用例迁入 `internal/chart/migrated_test.go`；② 删除 31 个已无生产调用方的根包私有 facade（否则它们保留 100% 覆盖而 `internal/chart` 仍记 0%）；③ 新增 `internal/chart/codec_test.go`（build→parse 往返 / CHART-02 扩展往返 / 校验分支 / fragment 构造器 / xlsx 包装配 / 实体解码）。结果：`internal/chart` **91.4%**（33.0% → 91.4%），全仓 total 回到 **84.4%**，与迁移前一致。`internal/chart` 不在 COV-04 低层格式包清单内（该清单为 opc / xmlstore / videoprobe / audioprobe / textmap / editplan），其 91.4% 属行为覆盖而非门槛要求。
...
- **Fourth refresh (2026-09-16, Tier 2 coverage-gate regression)**: `internal/opc` had silently fallen to **89.5%** after ADR-018 Tier 2 landed (`752fb95`) — the new `tryRawCopyOriginal` safety-gate reject branches (non-XML / Store-or-Deflate only / no encrypted bit / no data-descriptor bit / known size) shipped with **zero test coverage**, pulling the package below the COV-04 90% threshold. Attributed with isolated `git worktree` runs at `f7c8dad` (90.4%) vs `27a539d` (89.6%) — the drop came from Tier 2 itself, not from the later ADR-024 structural fix. Added `saveplan_raw_guard_test.go` (hand-built ZIP frames exercising exotic method / flags / zero-declared-size, an end-to-end fallback case, and write-error injection): `internal/opc` **89.5% → 90.3%**, threshold restored. Full-repo total unchanged at 84.4%.
...
| `internal/engine` | 85% | v2.0 编排层（CLI/WASM 共享 + ProjectIR adapter，实测 91.1%；图表投影已改走 ooxml schema，零门面句柄读取） |
| `internal/archlint` | 85% | v2.0 CI 依赖方向校验（实测 100%） |
| `internal/ooxml/schema` | 90% | v2.0 Step 1 XSD→Go 生成（实测 100%） |
| `internal/ooxml` | 90% | v2.0 格式层只读投影（Open/Bytes + SlideShapes 形状/文本/表格 + notes/timing/hidden + chart 投影，实测 92.7%） |
...
**FAIL**。动机：`FLOORS` 是白名单，未列包**静默不受检**——v2.0 试点期新增
`internal/textutil`/`bind`/`style`/`ooxmlns` 时曾三次差点漏加门槛。当前 `SKIP`：
`internal/document`（纯接口声明）、`internal/document/model`（纯标识类型）、
`internal/ooxmlns`（纯常量）、`scripts/gen_audio`/`gen_media`（`package main` 工具）。
...
**COV-04 closed 2026-09-12** (third refresh, commit `b676bab`): the flat 90%-per-package target remains abandoned for root SDK (still 85%, currently 82.8% with diminishing returns), but the six low-level format packages now satisfy per-package 90% — `videoprobe` 92.6%, `xmlstore` 90.8%, `editplan` 100%, `textmap` 100%, `opc` 90.0% (→ **90.3%** after the 2026-09-16 Fourth refresh). `audioprobe` stays at 88.4% per the documented B-2 decision. The Conclusion retains 90% for low-level format, 85% for root SDK, 85% for command/helper (all met), and behavior-based for `ir` (87.0%) / `render` (84.2%).
...
The 2026-09-16 regression (ADR-018 Tier 2 safety-gate branches, see Fourth refresh) is a reminder that **newly added code can silently cross a previously closed threshold** — a coverage check belongs in the definition of done for performance / IO changes, not just `go test` + `go vet`.
