...
| 3 | PowerPoint/WPS 关键用例实际打开无修复提示；**配音功能必须有播放记录** | ✅ **已闭合（2026-09-16）** | 打开/重存：PowerPoint 16.0.20326 + WPS 12.1.0.28599 × 4 样本 = 8/8（无修复提示、重存后 `Validate` errorCount=0），Tier 2 产物另行重跑 8/8；**配音播放记录**：用 SDK 制作含配音样本 → 期间查获并修复**两处 OOXML 合规缺陷**（[ADR-025](adr/ADR-025-audio-ooxml-compliance-fix.md)，PowerPoint 曾直接判包损坏 `0x80070570`）→ 修复后两家客户端均 `OPEN=ok`+`SAVE=ok`、音频识别为 `type=16 (msoMedia)`、`advanceTime=2500` 被实际采用（CreateVideo 导出时长 2.508s 佐证）→ **人工录屏留证**：录屏 `.mp4` 含 `soun` 轨（838 AAC 帧 / ≈126 kbps），**人耳确认可听到声音**。详见 [`docs/client-compat-matrix.md`](client-compat-matrix.md) 第二~四轮 |
