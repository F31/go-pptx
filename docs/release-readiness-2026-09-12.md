...
### 2.2 技术方案 V2.6 §15.3 发布硬门槛：**5 闭合 / 0 部分闭合**（第 3 条于 2026-09-16 闭合）
...
| 3 | PowerPoint/WPS 关键用例实际打开无修复提示；**配音功能必须有播放记录** | ✅ **已闭合（2026-09-16）** | 打开/重存：PowerPoint 16.0.20326 + WPS 12.1.0.28599 × 4 样本 = 8/8（无修复提示、重存后 `Validate` errorCount=0），Tier 2 产物另行重跑 8/8；**配音播放记录**：用 SDK 制作含配音样本 → 期间查获并修复**两处 OOXML 合规缺陷**（[ADR-025](adr/ADR-025-audio-ooxml-compliance-fix.md)，PowerPoint 曾直接判包损坏 `0x80070570`）→ 修复后两家客户端均 `OPEN=ok`+`SAVE=ok`、音频识别为 `type=16 (msoMedia)`、`advanceTime=2500` 被实际采用（CreateVideo 导出时长 2.508s 佐证）→ **人工录屏留证**：录屏 `.mp4` 含 `soun` 轨（838 AAC 帧 / ≈126 kbps），**人耳确认可听到声音**。详见 [`docs/client-compat-matrix.md`](client-compat-matrix.md) 第二~四轮。**公开样本复现（2026-09-16）**：公开可再分发样本 `s004-audio` 经 [ADR-027](adr/ADR-027-audio-visible-geometry-fix.md) 系列四项修复后，**用户真机确认 PowerPoint + WPS 两家 F5 均自动出声**（第五轮） |
...
> ~~关于第 3 条：这是唯一实质未闭合的硬门槛，性质是环境型缺口（缺音频语料 + COM 自动化无法验证播放效果），不是代码缺陷~~ —— **该判断已于 2026-09-16 被实测推翻并在同日闭合**：
> 1. 补出含配音样本后立刻发现 **PowerPoint 直接拒收**配音产物（`0x80070570 文件或目录损坏`），而同机同会话下原样本与 Tier 2 产物均能打开 —— 所以它**不是环境缺口，而是真实代码缺陷**（两处 OOXML schema 违反），且 WPS 比 PowerPoint 宽容，故一直未被发现；
> 2. 修复见 [ADR-025](adr/ADR-025-audio-ooxml-compliance-fix.md)（含读取侧三处同源修复与注入验证过的守门测试）；
> 3. 修复后完成真机打开、形状识别、计时生效三项验证，并以**人工录屏**（含 `soun` 轨、人耳确认有声）留下"播放记录"。
>
> **§15.3 五条发布硬门槛至此全部闭合。**
...
**A. 真缺口（建议 1.x 早期补）**
1. ~~**配音真机播放记录**（V2.6 §15.3 第 3 条）~~——**已闭合（2026-09-16）**：补出含配音样本 → 查获并修复两处 OOXML 合规缺陷（ADR-025）→ 人工录屏留证 → 公开音频样本 `s004-audio` 已创建（`scripts/gen_audio/main.go`），待真机验证。
