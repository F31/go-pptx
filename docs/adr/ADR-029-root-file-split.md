...
## 续二：文件名域前缀归一化（2026-09-16）

拆分到 63 个非测试文件后，根目录 `*.go` 达 **113 个**（63 源码 + 50 测试），
导航成本上升。评估后排除两条路径并选定本方案：

- **排除「按组件分子目录」**：Go 子目录即独立包——同包拆目录不可行，异包则须
  搬动全部公共类型（`Presentation`/`Slide`/…）→ 换 import 路径 = 破坏公共 API
  （ADR-015 冻结），且抽 `internal/` 触发 ADR-016 覆盖率归属陷阱。属 **v2.0
  大版本重构**范畴。
- **排除「合并小文件」**：与拆分目标（平均行数 342）相悖。
- **选定「同包内域前缀归一化」**：不改 package、不碰 API/行为/覆盖率，仅改名。

执行（`git mv`，55 个 rename = 38 源码 + 17 测试）：

- **core（无前缀，18 文件）**：入口/稳定类型保持简短名（presentation/slide/
  shape/save/options/errors/ids/…，`layoutdiag.go`→`layout_report.go`）。
- **10 个功能域前缀**：`text_*`(9) `style_*`(5) `theme_*`(2) `geom_*`(5)
  `format_*`(4) `bind_*`(6) `media_*`(6) `table_*`(2) `chart_*`(4) `anim_*`(2)。
- **测试文件与被测源文件同步改名**（如 `audio_test.go`→`media_audio_test.go`）。
- 规避 `_test.go` / GOOS / GOARCH 后缀冲突（改名前后均已核查，无构建约束文件）。
- `doc.go` 增加「文件组织」索引（域 × 文件清单，即本注记的可浏览版）。

守恒验证：源码文件数 63→63、测试 50→50、声明计数逐文件不变（纯改名）；
`go build` / `go test ./...` / `-tags=corpus` 14/14 全绿；覆盖率门禁 PASS。

> 历史文档（release notes / bug registry / 设计方案）中按旧文件名的引用
> 属当时记录，**不改写**；后续新代码按本归一化规则命名。
