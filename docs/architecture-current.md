...
| Chart implementation | XML read/write/canonical validation/workbook logic is concentrated in large files | **第一批+第二批已落地 (2026-09-12 ADR-017 r1+r2)**：3 真零依赖函数 + 4 常量搬到 `internal/chart`（BuildChartFrameFragment / ChartNumber / WorkbookColumn / GraphicURI / SheetName / CatAxID / ValAxID）+ 11 值对象 + Optional[T] 用 type alias 形式 move（ChartType / ChartSpec / ChartSeries / ChartData / ChartDataLabel / ChartErrorType / ChartErrorBars / ChartTrendType / ChartTrendline / ChartAxisOptions + Optional）；根包改薄包装 + alias，公共 API 表面零变化（// Stable: 34 / // Experimental: 5 / 158 总 type 不变），B1 黄金语料 replay 全绿，internal/chart 覆盖率 97.6%。第三批（30 函数 parse/build/canonical/validate 全搬迁）按 ADR-017 r1 排期 |
