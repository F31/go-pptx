...
- **分配总量应与媒体体积解耦**：Tier 1（流式复制）下 1×1 MiB、3×8 MiB 与
  **200×4 KiB** 应同为百 KiB 量级；若某档的 `分配总量/op` 涨到**与媒体体积同量级**，
  说明回退到「整个 Part 读进内存」的全缓冲路径。
- **`200x4KiB` 是「每 Part 固定开销」哨兵，不可删**：少而大的三档结构上不可能发现
  「对每个 Part 各分配一份缓冲/中间切片」这类退化（Tier 1 首版就栽在这里：小档语料
  保存分配量反向劣化 10×，只有本档能捕捉——实测无复用 7.24 MB vs 复用 231 KB，31×）。注意 `-benchtime=1x` 时一次性分配（构造包、zip writer 缓冲）被摊到单次迭代，
...
- **⚠️ 本机墙钟时间不可用于跨轮次比较（实测）**：本机为 Intel Core Ultra 9 275HX
  （**性能核 / 能效核混合架构**），Windows 线程调度在两类核之间的漂移会让
  **同一份代码、同一命令**的 `ns/op` 在不同轮次相差可达 **3.5×**
  （实测：`BenchmarkPerfOpen/100p-media` 同一 commit 先后两轮为 6.577 ms 与 1.9 ms）。
  因此：
  - **判定回归只能用确定性指标**（`B/op`、`allocs/op`、`peak-heap-B`）——它们在
    跨轮次间稳定到小数点后 2 位（如 `148810` vs `148912` B/op）；
  - 需要墙钟结论时，应在**同一进程内**做 A/B（或用 CI 的 Linux runner，无混合核调度问题）；
  - 本报告的时间列仅供同轮次内横向比较，**不要与历史报告的时间列做减法**。
