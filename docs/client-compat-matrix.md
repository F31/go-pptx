...
日期：2026-09-11（**真机首轮执行完成**）/ 2026-09-16（**第二轮：ADR-018 Tier 2 产物验证**；**第三轮：CreateVideo 对照实验**；**第四轮：配音录屏证据 → §15.3 第 3 条闭合**）；自动化脚本见 `scripts/l3/`
...
3. **确认结果（2026-09-16）**：
   - **音频源 = 麦克风**（非系统声音）；
   - **人耳确认：可听到声音**。
   - 证据链：`go-pptx AddAudio 产物` → PowerPoint 放映 → **扬声器外放** → 麦克风捕获 → 录屏音轨（838 帧 / ≈126 kbps）→ 回放可闻。属**间接但有效**的证据——麦克风路径经过空气传播，反而排除了"仅软件渲染、实际无输出"的可能。
   - 局限：未逐一确认所闻即那 2s 的 440 Hz 提示音；且依赖当时环境无其他声源。
   - **§15.3 第 3 条据此闭合**（见文末"发布硬门槛"）。
...
## V2.6 §15.3 发布硬门槛状态（2026-09-16 更新）

| # | 硬门槛 | 状态 |
|---|---|---|
| 1 | 金样操作通过结构断言及再次解析，输出不新增校验错误 | ✅ |
| 2 | 未修改 Part 的解压内容哈希一致（B1） | ✅ |
| 3 | 客户端实际打开无修复提示；**配音功能必须有播放记录** | ✅ **2026-09-16 闭合**（见第二~四轮：打开 8/8 → 查获并修复两处 OOXML 合规缺陷 → 形状识别 `type=16` + 计时生效 → 人工录屏含 `soun` 轨且人耳确认可闻） |
| 4 | 高严重度数据丢失 / 悬空引用 / 保存损坏 / 安全缺陷为零 | ✅ |
