...
- [x] CLONE-02（M8 收尾项，跨文档受限复制）：`Presentation.CopySlideFrom(src Slide, *ClonePolicy)`——源页全部内容（形状、文本、表格、图表、音频、备注、计时树、切换）复制为追加到当前文档 sldIdLst 末尾的新页面，返回新句柄。媒体总是独立复制（跨包无法共享字节，policy.IndependentMedia 被忽略）。**版式/母版按内容字节完全一致匹配复用**（matchReusePart）：目标已有相同 slideLayout / notesMaster 则复用，否则整体拒绝 ErrUnsupportedEdit（AT-13 跨包口径，零残留，不克隆版式/母版链）。与同文档 Clone 的差别：clonePlan 引入 srcDoc/crossDoc 分离，patchedCloneRels/cloneWalkContainer/applyClonePlan 全部参数化走 srcDoc；src.p == dst 时仍 dispatch 回同文档 Clone（语义更完整）。未知内部关系 / notesSlide 回引非源页 / 嵌入工作簿自带关系流等结构异常整体拒绝。
...
- [x] **TIMIR-01（M7 第三项）**：动画时序只读 IR（方案 §21.5）——`Slide.TimingTreeRaw()` 返回 p:timing 元素原始字节；`ir` 包把 p:timing → tnLst → par/cTn 树投影为只读时序 IR（`PageTiming{TimingNode 树, Totals, Diagnostics, OpaqueCount}`；TimingNode 覆盖并行/序列/cTn/audio/video/cmd/anim/set/opaque，含触发条件 Begin/End 与动画目标 Target）；未识别动画子元素输出 OpaqueNode 并计入诊断（不猜测、不省略）。**解析底层**：基于 `internal/xmlstore.Index`（Scanner 自闭合语义处理正确）构建节点索引树——彻底消除 std encoding/xml 在「自闭合 cond 列表后紧邻空 childTnLst」组合上的 depth 同步偏差，解析错误携带字节偏移。**R 档只读**——不提供动画编辑 API；估计值 Duration 不承诺与 Office 客户端播放帧一致，且不得作为 AdvanceAfter 计算输入（§21.5 不改变 PlanTimingSync 结论）
