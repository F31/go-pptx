...
- **V2.6 §15.3 五条发布硬门槛：全部闭合（2026-09-16）**。第 3 条（配音播放）原判"环境型缺口、非代码缺陷"**被实测推翻**：补出含配音样本后 **PowerPoint 直接拒收产物**（`0x80070570`，对照实验排除环境；WPS 宽容所以三层验证都没暴露）。根因 = 两处 OOXML schema 违反 + 三处读取侧同源问题，**已全部修复（ADR-025）**：① `buildAudioPicFragment`（`audio.go`）缺必需的 `p:nvPr`、`a:audioFile` 缺必需 `r:link` 且位置错（应在 `p:nvPr` 内）；② `SetAdvanceAfter` / `AdvanceAfter` 看不见 `mc:AlternateContent` 包裹的既有 `p:transition`（写侧会追加第二个 → 违反 `CT_Slide` maxOccurs=1；读侧读不到 `advTm`）；③ `classifyShape` 不填 AudioShape 的 profile、`picMediaKind` 只探 `blipFill`、`lastProfileByMedia` 漏读字段。修复后 PowerPoint + WPS 均 `OPEN=ok`/`SAVE=ok`、音频识别为 `type=16 (msoMedia)`、`advanceTime` 被实际采用（CreateVideo 导出时长 2.508s 佐证）；**人工录屏**含 `soun` 轨（838 AAC 帧 / ≈126 kbps）且人耳确认有声 → 第 3 条闭合。守门 `audio_ooxml_compliance_test.go`（3 条，均注入验证过必红）。L3 真机矩阵（非音频）两轮 8/8；详见 `docs/client-compat-matrix.md`（四轮 + 门槛表）。
...
- **制作含配音的 PPTX 样本**（§15.3 第 3 条语料）：SDK 三步即可——`slide.AddAudio(ctx, pptx.BytesMedia(synthWAV, "audio/wav"), AudioSpec{TrackKey, Role: AudioRoleNarration})` → `as.SetPlayback(PlaybackSpec{Trigger: PlaybackOnSlideEnter})` → `slide.SetAdvanceAfter(dur+padding)` → `Save`。音频**用代码合成 WAV**（自包含、可再分发、无外部依赖）；时长由 probe 推导（WAV/MP3 均可）。**两个前提**：① 模板须**无动画**（`SetPlayback` 对含非纯音频 timing 的页返回 `ErrTimingConflict`，AT-05）；② 生成器用 `//go:build ignore` + `go run <file>` 模式（与 `scripts/gen_corpus/gold_replace.go` 一致）。样例工具：`.l3-output/zz_genaudio.go`（生成）、`.l3-output/zz_dumpzip.go`（ZIP 条目 + `-find` 窗口取证，XML 单行很长时必需）。**PowerPoint 对包结构远比 WPS 严格**——凡改 OOXML 结构必须真机验 PowerPoint，不可只用 WPS 或代码级测试。
