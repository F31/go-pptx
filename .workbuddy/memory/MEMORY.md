...
- **V2.6 §15.3 第 3 条（配音播放）——性质已于 2026-09-16 修正**：不再是"环境型缺口（缺 audio 语料）"。做出含配音样本后实测发现 **PowerPoint 拒收 go-pptx 的音频产物**（`0x80070570 文件或目录损坏`，对照实验排除环境因素：原样本与 Tier 2 产物均 `OPEN=ok`），而 WPS 正常（识别 `Audio 11`、`advanceTime=2.5` 生效）。根因（源码已定位）：`buildAudioPicFragment`（`audio.go:466`）两处 OOXML schema 违反——`p:nvPicPr` 缺必需的 `p:nvPr`；`a:audioFile` 缺必需的 `r:link` 且应放 `p:nvPr` 内（现放在 `p:blipFill` 内）。**待立项修复**；修前该产物不可用于真机验证。L3 客户端矩阵（非音频用例）已闭合（真机 8/8 + Tier 2 产物 8/8，`docs/client-compat-matrix.md`）。
